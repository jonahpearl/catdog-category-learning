...
%             histogram(abs(signf_coeffs{i}), [0:0.05:1.2]) % confirm absolute num is higher post -- yes.
        ...
        % Stats testing if only two sessions (pre and post)
        [p, h] = ranksum(abs(signf_coeffs{1}), abs(signf_coeffs{2}), 'tail', 'left');
        if numel(rSessions)==2 && (p < ranksum_alpha)
            fprintf('%s, median of GLM coeffs signf higher session %d > %d \n',...
                Monkeys(m).Name, rSessions(1), rSessions(2))
        else
            fprintf('%s, median of GLM coeffs no signf diff session %d and %d \n',...
                Monkeys(m).Name, rSessions(1), rSessions(2))
        end
