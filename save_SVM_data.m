...
function save_SVM_data(data, paramStruct, recordMatPath)
% 
...
for f = 1:length(fields)
    if strcmp(fields{f}, 'ID')
        ...
        Records(len+1).fields(f) = paramStruct.fields(f);
    end
...
save(sprintf('SVM_results_%d.mat', id), 'data');
