...
%% Fig 4A prep: cluster-based permutation statistics
...
%% Fig 4A prep: Check when timecourse first signf above shuffle
...
%% Fig 4B prep: params and load data
...
%% Fig 4B: plot timecourse across arrays 
...
%% Fig 4C prep: load data

% Load data
% ID = 316514; % all sessions, [10 25 50 100], {[175 275]}, no shuffle.
% ID = 331368;  % ^ but matched trial nums
ID = 291617;  % ^ but with 240/240 imgs
load(sprintf(fullSVMPath, ID), 'data');
SVM = data;
clear data
...
%% Fig 4C: plot a given subset/interval over sessions

rSessionsByMonk = {[1 2 3 5 6 7 9], 1:7};  % Fig 2C
top_n_units = 100;
interval_to_plot = [175 275];
% kfl_base_name = sprintf('KFL_SingleUnitSVMRanking_sparsa_Top_%d', top_n_units);  % top 10, 25, 50, or 100
kfl_base_name = sprintf('KFL_SingleUnitSVMRanking_sparsa_Top_%d_TrNMatch', top_n_units);
% kfl_base_name = sprintf('KFL_SUnSVMRk_sprs_T_%d_TrNMch', top_n_units);

figure2('Position', [400 100 240 500])
mkYLims = {[0.675 0.8], [0.55 0.62]};
gray = [0.5 0.5 0.5];
...
for m = 1:length(Monkeys)
    ...
    for iLoc = 1:length(rArrayLocs)
        ...
        ylim(mkYLims{m})
        ylabel("Accuracy")
        xlabel("Passive viewing session")
...
% kfl_base_name = "KFL_SingleUnitSVMRanking_sparsa_Top_%d_TrNMatch";
rArrayLocs = {'te'};
num_best_units_to_use = [1:1:50 52:2:100];

manualIntervals = {[175 275]};
rSessionsByMonk = {[7 9], [6 7]};  % (Fig 2B / 2D)

%% Fig 4E prep: plot raw performance wrt adding units
...
%% Fig 4E: curve fitting for SVM while adding units
...
%                     'HandleVisibility', 'off');
                ...
                c_ints = confint(f);
                fprintf("\t %0.3g - %0.3g \n \t %0.3g - %0.3g \n \t %0.3g - %0.3g \n \t %0.3g - %0.3g \n",...
                    c_ints(1,1), c_ints(2,1),...
                    c_ints(1,2), c_ints(2,2),...
                    c_ints(1,3), c_ints(2,3),...
                    c_ints(1,4), c_ints(2,4))
            end
            set(gca, 'XScale', 'log')
            xlabel('Num best units used')
            ylabel('Accuracy')
...
%% Fig 4D: see pv_SVM_neuralSubsets_initial2020.m
